...
public struct ChartResponse: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let chartCointainer = try? container.nestedContainer(keyedBy: ChartKeys.self, forKey: .chart) {
            data = try? chartCointainer.decodeIfPresent([ChartData].self, forKey: .result)
            error = try? chartCointainer.decodeIfPresent(ErrorResponse.self, forKey: .error)
        } else {
            data = nil
            error = nil
        }
...
public struct ChartData: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let indicatorsContainer = try? container.nestedContainer(keyedBy: IndicatorsKeys.self, forKey: .indicators), var quotes = try? indicatorsContainer.nestedUnkeyedContainer(forKey: .quote), let quoteContainer = try? quotes.nestedContainer(keyedBy: QuoteKeys.self) {
